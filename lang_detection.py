...
#         return []
#
#     # ---- Keep full cell text as single segment ----
#     # Splitting into word tokens loses short words (de, la, el)
#     # and causes bbox mismatches. The cell bbox already covers
#     # the entire phrase correctly.
#
#     return [{
#         "word":                original_text.strip(),
#         "original_token":      original_text.strip(),
#         "source_type":         record.get("source_type"),
#         "source_language":     record.get("source_language"),
#         "language_confidence": record.get("language_confidence"),
#         "page_no":             record.get("page_no"),
...
def enrich_segments_with_word_bboxes(segments: List[dict], file_path: str) -> List[dict]:
    ...
    for seg in segments:
        ...
        # for idx, fw in enumerate(page_word_list):
...
#         return re.sub(r"^[\W]+|[\W]+$", "", s, flags=re.UNICODE)
#
#     # ✅ FIX: overlap instead of strict containment
#     def overlaps(fx0, fy0, fx1, fy1, cx0, cy0, cx1, cy1, tol=2.0):
#         return not (
#             fx1 < cx0 - tol or
#             fx0 > cx1 + tol or
#             fy1 < cy0 - tol or
...
#         )
#
#     # ✅ Group segments by cell (page, row, column)
#     grouped = defaultdict(list)
#     for seg in segments:
#         key = (seg.get("page_no"), seg.get("row_no"), seg.get("column_no"))
#         grouped[key].append(seg)
#
#     enriched = []
#
#     for (page_no, row_no, column_no), group in grouped.items():
#
#         if page_no is None:
...
#             match_token = seg.get("original_token") or seg.get("word", "")
#
#             # multi-word: cell bbox is already correct
#             if len(match_token.split()) > 1:
#                 enriched.append(seg)
#                 continue
#
...
#
#             for idx, fw in enumerate(page_word_list):
#                 fx0, fy0, fx1, fy1, fword = fw[:5]
#
#                 if not overlaps(fx0, fy0, fx1, fy1, cell_x0, cell_y0, cell_x1, cell_y1):
#                     continue
#
#                 fw_clean = strip_punct(fword).lower()
#                 # parts = re.split(r"[\/|,-]", fw_clean)
#
#                 # direct single word match
#                 if fw_clean == token_norm and idx not in used_indices:
#                     best = (fx0, fy0, fx1, fy1)
#                     used_indices.add(idx)
#                     break
#
#                 # # multi-part word match — do NOT add to used_indices so next part can also match
#                 # if token_norm in parts:
#                 #     part_index = parts.index(token_norm)
#                 #     part_width = (fx1 - fx0) / len(parts)
#                 #     best = (
#                 #         fx0 + part_width * part_index,
#                 #         fy0,
#                 #         fx0 + part_width * (part_index + 1),
#                 #         fy1
#                 #     )
#                 #     break  # ← no used_indices.add() here
#
#             if best is not None:
#                 seg = {**seg, "x0": best[0], "y0": best[1], "x1": best[2], "y1": best[3]}
#             else:
#                 print("MISS:", match_token, "| PAGE:", page_no,
#                       "| ROW:", seg.get("row_no"),
#                       "| CELL BBOX:", (cell_x0, cell_y0, cell_x1, cell_y1))
#
#             enriched.append(seg)
