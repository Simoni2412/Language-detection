...
IGNORE_TERMS = {"unk", "n/a", "na", "ni", "yes", "no", "fr", "comment", "comments", "sender",
                ...
                "given", "value/qualifier", "duplicate", "textual", "medra", "unit", "paclitaxel",
...
def extract_foreign_content(
    ...
    for item in parsed_data.get("table_data", []):
        ...
        print("before prepareeee", text)
        detect_text = prepare_for_detection(text)
        print("debuugsssss", detect_text)
        if not detect_text:
            continue
...
def segment_foreign_record(record: dict) -> List[dict]:
    ...
    # print("highlight_text:", original_text)
    highlight_text = re.sub(r"\b\d+\s*mg/mL\b", " ", original_text, flags=re.I)
    ...
    for token in highlight_text.split():
        ...
        if len(clean) > 4:
            ...
            if lang is not None:
                ...
                if  lang.iso_code_639_1.name.lower() == "en" or (lang.iso_code_639_1.name.lower() !="en" and conf<0.60):
